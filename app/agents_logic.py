...
class Position:
    def __init__(self, symbol, side, entry_price, amount, tp_price, sl_price, timestamp):
        self.symbol = symbol
        self.side = side
...
class VolatilityModel(MLModel):
    def predict(self, md):
        ...
        if rsi < 40: return 1 # More sensitive
        if rsi > 60: return -1
        return 0
...
class TradingAgent(ABC):
    ...
    async def close_position(self, exit_price, timestamp, reason):
        ...
        leverage = 10
        profit_loss = (pos.amount * profit_pct / 100) * leverage
        ...
        trade_record = {
            ...
            "profit_pct": round(float(profit_pct * leverage), 2),
...
class IntelligenceAgent(TradingAgent):
    def decide_trade(self, md):
        ...
        rsi = float(calculate_rsi(closes)[-1])
        ...
        self.active_position = Position(self.symbol, side, current_price, amount, tp, sl, datetime.now().isoformat())
        return {"action": "ENTRY", "confidence": confidence}
...
class SimulationEngine:
    ...
    async def step(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            if agent.active_position:
                ...
                hit_tp = (pos.side == "LONG" and float(current_candle['high']) >= pos.tp_price) or \
                         (pos.side == "SHORT" and float(current_candle['low']) <= pos.tp_price)
                hit_sl = (pos.side == "LONG" and float(current_candle['low']) <= pos.sl_price) or \
                         (pos.side == "SHORT" and float(current_candle['high']) >= pos.sl_price)

                if hit_tp: await agent.close_position(pos.tp_price, datetime.now().isoformat(), "TP Hit (Ensemble Logic)")
                elif hit_sl: await agent.close_position(pos.sl_price, datetime.now().isoformat(), "SL Hit (Safety Trigger)")

            agent.decide_trade(md)
        self.save_state()
    ...
    def save_state(self):
        def pos_to_dict(p):
            ...
            return {
                "symbol": p.symbol, "side": p.side, "entry_price": float(p.entry_price),
                "amount": float(p.amount), "tp_price": float(p.tp_price), "sl_price": float(p.sl_price),
                "entry_time": p.entry_time
            }
        state = [{"name": a.name, "strategy": a.strategy_name, "balance": float(a.balance),
                  "is_active": bool(a.is_active), "trades": a.trades, "active_position": pos_to_dict(a.active_position)} for a in self.agents]
            ...
            with open(STATE_FILE, 'r') as f:
                ...
                for i, s in enumerate(state):
                    if i < len(self.agents):
                        ...
                        if ap: a.active_position = Position(ap['symbol'], ap['side'], ap['entry_price'], ap['amount'], ap['tp_price'], ap['sl_price'], ap['entry_time'])
        except Exception: pass

    def get_status(self):
        return [{
            "name": a.name, "strategy": a.strategy_name, "symbol": a.symbol,
            "balance": round(float(a.balance), 2), "is_active": bool(a.is_active),
            "trade_count": len(a.trades), "last_trade": a.trades[-1] if a.trades else None,
            "active_position": {"side": a.active_position.side, "entry": round(float(a.active_position.entry_price), 4), "tp": round(float(a.active_position.tp_price), 4)} if a.active_position else None
