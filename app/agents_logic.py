...
class Position:
    def __init__(self, side: str, entry_price: float, leverage: int = 10, confidence: float = 0):
        ...
        # Bottom-Hunting aggressive targets
        tp_dist = 0.020 # 2% move
        sl_dist = 0.008 # 0.8% move
...
class IntelligenceAgent:
    ...
    def get_ml_insights(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "importance": importance_dict, "insight": "Shared Brain optimizing for high-precision LONG reversals."
            }
    ...
    def predict(self, df: pd.DataFrame) -> Dict:
        ...
        if not self.is_trained:
            ...
            if rsi < 15: return {"side": "LONG", "confidence": 80.0} # Aggressive early bottom hunt
...
class TradingAgent:
    def __init__(self, name: str, balance: float = 1000.0):
        ...
        self.trades = []
        self.leverage = 10
        self.is_special = True # All agents are now special Wick-Hunters
    ...
    def tick(self, df: pd.DataFrame, shared_brain: IntelligenceAgent):
        ...
        if self.active_position:
            ...
            if self.active_position.side == "LONG":
                if pnl_high >= 0.20: should_exit = True; exit_reason = "Take Profit"; exit_price = self.active_position.tp_price
                elif pnl_low <= -0.08: should_exit = True; exit_reason = "Stop Loss"; exit_price = self.active_position.sl_price
                elif prediction['side'] == "SHORT" and prediction['confidence'] > 75: should_exit = True; exit_reason = "Brain Reversal"
            else:
                if pnl_low >= 0.20: should_exit = True; exit_reason = "Take Profit"; exit_price = self.active_position.tp_price
                elif pnl_high <= -0.08: should_exit = True; exit_reason = "Stop Loss"; exit_price = self.active_position.sl_price
                elif prediction['side'] == "LONG" and prediction['confidence'] > 75: should_exit = True; exit_reason = "Brain Reversal"
            ...
            can_enter = False; conf = 0; side = "NONE"

            # Global Bottom Hunter logic for ALL agents
            # High precision triggers: RSI < 20 and StochK < 10 (Deep Bottom)
            if last_row['rsi'] < 20 and last_row['stoch_k'] < 10:
                can_enter = True; side = "LONG"; conf = 90.0
            elif prediction['side'] != "NONE" and prediction['confidence'] >= 75:
